...
!(async () => {
  ...
  // 动态计算超时时间：基础3秒 + 每MB增加2秒
  const downloadTimeout = 3000 + mb * 2000
  ...
  const res = await $.http.get({
    ...
    timeout: downloadTimeout  // 使用动态计算的超时
  })
  const end = Date.now()
  ...
  const ping = await $.http.get({
    ...
    node: $environment.params.node,
    timeout: 3000  // ping请求保持3秒超时
  })
  pingt = Date.now() - pingstart
  console.log('延迟时间:' + pingt + 'ms')
  console.log('下载耗时:' + duration + 's')
  ...
  icon = shifts[a]
  color = shifts[b]
  console.log(`图标: shifts[${a}]:` + shifts[a])
  console.log(`颜色: shifts[${b}]:` + shifts[b])
  
  // 标题改为中文"网络测速"，允许用户自定义覆盖
